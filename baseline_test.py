...
def pick_samples(df):
	...
	# for token_length in [(5000, 10000)]: # for testing
		filtered_df = df[(df['token_count'] >= token_length[0]) & 
						 (df['token_count'] <= token_length[1])]

		if len(filtered_df) < sample_size:
			logger.info(f"Not enough samples in token range {token_length}")
			continue
				...
				for code_type in ['vuln_code', 'non_vuln_code']:
						...
						try:
								...
								for m, model in enumerate(models):
										# Log progress
										sample_index = s + (1 if code_type == 'non_vuln_code' else 0) + (m + 1) + 1
...
def main():
	for l, lang in enumerate(langs):
		logger.info(f'[Log] {l+1}/{len(langs)}|Language:{lang}')
		query = f"""
			SELECT
		...
		if len(df) <= 100: 
			logger.info(f'[INFO] {lang} will be skipped')
			break
		...
		with open(f'./baseline/Baseline_{lang}.pkl', 'wb') as f:
