...
from variables import SYSTEM_PROMPT, COT_PROMPT, FS_PROMPT, MODELS
...
def main():
    ...
    with open(f"./dataset/new_test/test_df.pkl", "rb") as f:
        df = pickle.load(f)

    timestamp = datetime.now().strftime("%m-%d_%H-%M")
    for model in MODELS:
        for prompt_template, prompt_type in zip([COT_PROMPT, FS_PROMPT], ['cot_prompt', 'fs_prompt']):
            ...
            with open(f"{folder_path}/final_results_{model}.json", "w", encoding="utf-8") as f:
